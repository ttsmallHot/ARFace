...
namespace XCSJ.EditorSMS.States.TimeLine
{
    ...
    public class StateWorkClipSetInspector : WorkClipInspector<StateWorkClipSet>
    {
        ...
        /// </summary>
        [Name("排序规则")]
        [Tip("对状态工作剪辑集合内的元素进行排序", "Sorts the elements within the state work clip collection")]
        public enum ESortRule
        {
            ...
            /// </summary>
            [Name("默认")]
            [Tip("按照\"状态机连线顺序\"功能生成的顺序队列,对当前列表进行排序;", "Sort the current list according to the sequence queue generated by the \"state machine connection sequence\" function;")]
            Default,
        ...
        public enum ELcokExtension
        {
            ...
            /// </summary>
            [Name("速度")]
            [Tip("当前状态在状态工作剪辑集合时间轴中的速度;修改速度,仅影响次数;", "The speed of the current state in the timeline of the working clip set; Modify the speed and only affect the number of times;")]
            [EnumFieldName("速度")]
            Speed = 1 << 0,
            ...
            /// </summary>
            [Name("次数")]
            [Tip("次数=时长*速度/单次时长;即在指定时长内循环的次数;修改次数,仅影响速度;", "Times = duration * speed / single duration; That is, the number of cycles within the specified time; The number of modifications only affects the speed;")]
            [EnumFieldName("次数")]
            LoopCount = 1 << 1,
        }
        ...
        [Name("单次时长=>时长")]
        [Tip("将'单次时长'设置为'时长'的对应数值", "Set 'single time length' to the corresponding value of 'time length'")]
        ...
        [Name("时长=>单次时长")]
        [Tip("将'时长'设置为'单次时长'的对应数值", "Set 'duration' to the corresponding value of 'single duration'")]
        ...
        {
            var totalTimeLength = stateWorkClipSet.timeLength;
                ...
                if (WorkClipEditor.Draw(target, rect, index, stateWorkClip, lockWorkClip, stateWorkClipSet.timeLength,
                    AfterHorizontalSpaceWidth, (inRect) =>
                    {
                        ...
                        if (option.speed)
                        {
                            ...
                            EditorGUI.BeginChangeCheck();
                            var speed = EditorGUI.DelayedDoubleField(inRect, GUIContent.none, stateWorkClip.speed);
                            if (EditorGUI.EndChangeCheck())
                            {
                                UndoHelper.RegisterCompleteObjectUndo(stateWorkClipSet);
                        ...
                        if (option.loopCount)
                        {
                            ...
                            EditorGUI.BeginChangeCheck();
                            loopCount = EditorGUI.DelayedDoubleField(inRect, GUIContent.none, loopCount);
                            if (EditorGUI.EndChangeCheck())
                            {
                                UndoHelper.RegisterCompleteObjectUndo(stateWorkClipSet);
